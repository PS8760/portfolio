...
export default function Footer() {
  ...
  const c = {
    ...
    border: isDark ? 'rgba(139,92,246,0.1)' : 'rgba(0,0,0,0.08)',
  ...
  return (
    <>
      {/* CTA Strip */}
      <div className="py-16 relative overflow-hidden" style={{ background: isDark ? '#0f0f1a' : '#f5f5f3' }}>
        ...
        <div className="max-w-3xl mx-auto px-6 text-center relative z-10">
          ...
          <button
            ...
            className="shimmer-btn inline-flex items-center gap-2 px-8 py-3.5 rounded-full font-semibold text-sm text-white cursor-pointer transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_12px_35px_rgba(139,92,246,0.45)]"
            style={{ border: 'none' }}>
      ...
      <footer style={{ background: c.surface, borderTop: `1px solid ${c.border}` }}>
        <div className="max-w-6xl mx-auto px-6 py-12 flex flex-wrap items-start justify-between gap-8">
            ...
            <div className="flex gap-2">
              {socials.map((s) => (
                <a key={s.label} href={s.href} target="_blank" rel="noreferrer" aria-label={s.label}
                  className="flex items-center justify-center w-8 h-8 rounded-lg border transition-all duration-200 hover:text-violet-400 hover:border-violet-500/40 hover:bg-violet-500/8 hover:-translate-y-0.5"
            ...
            <nav className="flex flex-col gap-1">
              {navLinks.map((link) => (
                <button key={link} onClick={() => scrollTo(link)}
                  className="text-left px-0 py-1.5 text-sm font-medium transition-colors cursor-pointer hover:text-violet-400"
          ...
          <div>
            ...
            <div className="flex flex-col gap-2 text-sm" style={{ color: c.muted }}>
              <a href={`mailto:${personalInfo.email}`} className="hover:text-violet-400 transition-colors" style={{ textDecoration: 'none', color: 'inherit' }}>
                {personalInfo.email}
              </a>
              <span>{personalInfo.location}</span>
              <a href={personalInfo.instagram} target="_blank" rel="noreferrer" className="hover:text-violet-400 transition-colors" style={{ textDecoration: 'none', color: 'inherit' }}>
                @ghodke.1984
              </a>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 text-xs flex-wrap gap-2"
          ...
          <span className="text-[10px] px-2.5 py-1 rounded-full" style={{ background: 'rgba(139,92,246,0.08)', color: '#8b5cf6' }}>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-50 w-11 h-11 rounded-full flex items-center justify-center cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_24px_rgba(139,92,246,0.5)]"
        style={{
